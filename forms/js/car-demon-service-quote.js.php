...
$x = '';
$hook_form_js = apply_filters('car_demon_mail_hook_js', $x, 'service_quote', 'unk');
$hook_form_js_data = apply_filters('car_demon_mail_hook_js_data', $x, 'service_quote', 'unk');
?>
// JavaScript Document
function car_demon_validate_service_quote(form_id) {
	...
	if (msg != "") {
		...
		var form_key = document.forms["service_quote"+form_id].form_key.value;
		<?php echo $hook_form_js; ?>
		jQuery.ajax({
			type: 'POST',
			data: {'your_name': your_name,'phone':phone, 'email':email, 'service_location':service_location, 'year':year, 'make':make, 'model':model, 'miles':miles, 'vin':vin, 'service_needed':service_needed, 'form_key':form_key<?php echo $hook_form_js_data; ?>},
			url: "<?php echo $car_demon_pluginpath; ?>theme-files/forms/car-demon-service-quote-handler.php?send_service=1",
			timeout: 2000,
			error: function() {},
			dataType: "html",
