jQuery("#select_stock_txt").autocomplete(
  cdFinanceParams.ajaxurl,
  {
		extraParams: {action:"cd_finance_find_stock"},
		delay:10,
		minChars:2,
		matchSubset:1,
		matchContains:1,
...
jQuery("#select_car_txt").autocomplete(
  cdFinanceParams.ajaxurl,
  {
		extraParams: {action:"cd_finance_find_vehicle"},
		delay:10,
		minChars:2,
		matchSubset:1,
		matchContains:1,
...
function selectItem(li) {
	...
	jQuery.ajax({
		type: 'POST',
		data: {action: 'cd_finance_show_stock', 'stock_num': stock_num, 'show_stock': 1},
		url: cdFinanceParams.ajaxurl,
		timeout: 2000,
		error: function() {},
		dataType: "html",
...
function selectCarItem(li) {
	...
	jQuery.ajax({
		type: 'POST',
		data: {action: 'cd_finance_show_stock', 'car_title': car_title, 'show_stock': 2},
		url: cdFinanceParams.ajaxurl,
		timeout: 2000,
		error: function() {},
		dataType: "html",
...
function mainValidation() {
    ...
    //=====VOI Validation
        ...
        if (no_car == 1) {
            ...
            if (selected_car == "") {
                var msg = msg + cdFinanceParams.error1;
...
function YesOrNo() {
    if (document.frm_app.co_fn2.disabled==True) {
        var r=confirm(cdFinanceParams.error3);
        if (r==true) {
            enableCoSigner();
